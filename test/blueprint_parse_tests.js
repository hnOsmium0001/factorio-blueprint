...
import { Blueprint, util } from '../dist/factorio-blueprint.min.js';
...
describe('Blueprint Parsing', () => {
  ...
  describe('directions', () => {
    it('supports belts going in all directions', () => {
      ...
      const bp = new Blueprint(input);
      const n = bp.findEntity(new Victor(0, -1));
      const e = bp.findEntity(new Victor(1, 0));
      const s = bp.findEntity(new Victor(0, 1));
      const w = bp.findEntity(new Victor(-1, 0));
  ...
  describe('recipes', () => {
    it('supports recipes in assemblers', () => {
      ...
      const bp = new Blueprint(input);
      const assembler = bp.findEntity(new Victor(0, 0));
  ...
  describe('modules', () => {
    it('supports modules in assemblers', () => {
      ...
      //assert.assert.equal(assembler.modules, {'speed_module_3': 1, 'productivity_module_3': 2, 'effectivity_module_3': 1}); // Should work out why this assertion does not pass.
    });
  });
  ...
  describe('circuit conditions', () => {
    it('can enable/disable train stops', () => {
      ...
      // XXX assert.assert.equal(train_stop.condition.constant, 0);
      // XXX assert.assert.equal(train_stop.condition.modes['send_to_train'], 'false');
    });
