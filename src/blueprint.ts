...
import { defaultEntities } from './default_entities';

export type Version = '0' | 'latest';
...
export class Blueprint {
  ...
  toObject() {
    ...
    const iconData = this.icons.map((icon, i) => {
      return {
        signal: { type: defaultEntities[icon].type, name: this.fixName(icon) },
        index: i + 1,
      };
    });
  ...
  static setEntityData(obj: any) {
    ...
    for (let i = 0; i < keys.length; i++) {
      defaultEntities[keys[i]] = obj[keys[i]];
    }
  }
  ...
  checkName(name: string) {
    ...
    if (!defaultEntities[name] && this.checkWithEntityData)
      throw new Error(
